...
class UserInputs:
    ...
    def material_input(self):
        choice = st.selectbox(
            "Material",
            ["AlGaAs", "AlGaSb", "InGaAs_InAlAs", "InGaAs_GaAsSb"]
        )
    ...
    def np_input(self, solver_type):
        np_options = (
            ["Parabolic", "Taylor", "Kane", "Ekenberg"]
            if solver_type == "TMM"
            else ["Parabolic", "Taylor", "Kane"]
        )
...
class EnergyDiffInputs(UserInputs):
    def render_energy_diff_inputs(self):
        ...
        self.sweep_param = st.pills(
            "Choose sweep parameter:",
            ["Sweep Well Width", "Sweep Molar Content"]
        )
    ...
    def get_sweep_ranges(self, graph_type):
        ...
        match graph_type:
            ...
            case "Sweep Molar Content":
                ...
                heights = list(np.arange(h_start, h_end, h_step))
                widths = [90]
